...
API_TOKEN = '6219528902:AAF7a-Yrg3yw25EaV4xBXCNXy3nltSDR0Fo' # токен, который выдается на бота
...
def next_state(id, state, text, keyboard):
    ...
    if state == UserStates.second_scale_end:
        ...
        result = users_data[id].systolic_blood_pressure + users_data[id].heart_rate + users_data[id].respiratory_rate
...
def start(message):
    ...
    bot.send_message(
        ...
        text=f'Привет, {message.from_user.first_name}, я бот для оценки риска развития и степени тяжести кровотечений '
             f'в родах. Используется догоспитальная шкала оценки тяжести кровотечений.\n\nНажми кнопку, чтобы начать. ',
...
def new_bleeding(call):
    ...
    # Выбираем сценарий второй шкалы
