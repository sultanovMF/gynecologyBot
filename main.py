...
# токен, который выдается на бота
...
systolic_blood_pressure_buttons = [
    [types.InlineKeyboardButton(
        text="Значения в пределах нормы", callback_data="CAD_NORM")],
    [types.InlineKeyboardButton(
        text="Снижение САД на 20-50 мм.рт.ст. ", callback_data="CAD_1")],
    [types.InlineKeyboardButton(
        text="Снижение САД на 50-80 мм.рт.ст. ", callback_data="CAD_2")],
    [types.InlineKeyboardButton(
        text="Снижение САД на более 80 мм.рт.ст. ", callback_data="CAD_3")],
    [types.InlineKeyboardButton(
        text="Начать заново", callback_data="START")]
]
systolic_blood_pressure_keyboard = types.InlineKeyboardMarkup(
    systolic_blood_pressure_buttons)

# Кнопка ЧСС
heart_rate_buttons = [
    [types.InlineKeyboardButton(
        text="Значения в пределах нормы (менее 100-110 уд/мин.):", callback_data="HR_NORM")],
    [types.InlineKeyboardButton(
        text="Повышение ЧСС на 1-20 уд/мин. (110-130 уд/мин.): ", callback_data="HR_1")],
    [types.InlineKeyboardButton(
        text="Повышение ЧСС на 20-40 уд/мин. (130-150 уд/мин.): ", callback_data="HR_2")],
    [types.InlineKeyboardButton(
        text="Повышение ЧСС на более 40 уд/мин. (более 150 уд/мин.): ", callback_data="HR_3")],
...
respiratory_rate_buttons = [
    [types.InlineKeyboardButton(
        text="ЧДД менее 26/мин",        callback_data="BR_1")],
    [types.InlineKeyboardButton(
        text="ЧДД 26-30/мин",           callback_data="BR_2")],
    [types.InlineKeyboardButton(
        text="ЧДД более 30/мин",        callback_data="BR_3")],
    [types.InlineKeyboardButton(
        text="Начать заново", callback_data="START")]
]
respiratory_rate_keyboard = types.InlineKeyboardMarkup(
    respiratory_rate_buttons)

# Кнопка диуреза
diuresis_buttons = [
    [types.InlineKeyboardButton(
        text="Значения в пределах нормы", callback_data="DIURES_NORM")],
    [types.InlineKeyboardButton(
        text="Диурез более 20-30 мл/ч", callback_data="DIURES_1")],
    [types.InlineKeyboardButton(
        text="Диурез 5-20 мл/ч", callback_data="DIURES_2")],
...
FIRST_SCALE_STATES = ((UserStates.body_mass, 'Введите массу (в кг) тела пациентки по примеру: 60.5', None),
                      (UserStates.body_height,
                       'Введите рост (в метрах) пациентки по примеру: 1.78 ', None),
                      (UserStates.systolic_blood_pressure,
                      ...
                      (UserStates.heart_rate, 'Изменение частоты сердечных сокращений(ЧСС)\n\nИли по копке можете вернуться в главное меню',
                       heart_rate_buttons_keyboard),
                      (UserStates.respiratory_rate,
                       'Изменение частоты дыхательных движений(ЧДД)\n\nИли по копке можете вернуться в главное меню', respiratory_rate_keyboard),
                      (UserStates.diuresis,
                       'Диурез (мл/ч)\n\nИли по копке можете вернуться в главное меню', diuresis_keyboard),
                      (UserStates.first_scale_end, '', None))

# Шкала без диуреза
SECOND_SCALE_STATES = ((UserStates.body_mass, 'Введите массу (в кг) тела пациентки по примеру: 60.5', None),
                       (UserStates.body_height,
                        'Введите рост (в метрах) пациентки по примеру: 1.78', None),
                       (UserStates.systolic_blood_pressure,
                        'Изменение систолического артериального давления (САД)\n\nИли по копке можете вернуться в главное меню', systolic_blood_pressure_keyboard),
...
def next_state(id, state, text, keyboard):
    ...
    if state == UserStates.first_scale_end:
        ...
        result = users_data[id].systolic_blood_pressure + users_data[id].heart_rate + \
            users_data[id].respiratory_rate + users_data[id].diures
        ...
        if 0 <= result <= 3:
            ...
            Vkp = (users_data[id].body_mass * 11,
                   users_data[id].body_mass * 30)
            text = f'''Физиологическая кровопотеря в размере 11-30%;\nДля данной пациентки Vкп (мл) составляет: ({Vkp[0]:.0f}, {Vkp[1]:.0f});\nПри ИМТ в пределах нормы, составляет 0-600 мл.
            '''
        elif 9 <= result <= 14:
            Vkp = (users_data[id].body_mass * 31,
                   users_data[id].body_mass * 100)
            text = f'''Массивная кровопотеря в размере 31% ОЦК и более;\nДля данной пациентки Vкп (мл) составляет: ({Vkp[0]:.0f}, {Vkp[1]:.0f});\nПри ИМТ в пределах нормы, составляет более 1800 мл.
            '''
        else:
            raise Exception(
                'Неверная логика работы программы при разбаловке первой шкалы')

        bot.send_message(id, text=text, reply_markup=keyboard)

    elif state == UserStates.second_scale_end:
        # Обработка результатов второй шкалы без диуреза

        if (users_data[id].body_mass and users_data[id].body_height and is_print_mass_body_index):
            mass_body_index = users_data[id].body_mass / \
                users_data[id].body_height / users_data[id].body_height
            if (16 <= mass_body_index <= 18.4):
                bot.send_message(
                    id, text=f'Индекс массы тела: дефицит массы тела')
            elif (18.4 < mass_body_index <= 24.9):
                bot.send_message(id, text=f'Индекс массы тела: норма')
            elif (24.9 < mass_body_index <= 29.9):
                bot.send_message(
                    id, text=f'Индекс массы тела: избыточная масса тела')
            elif (29.9 < mass_body_index <= 34.9):
                bot.send_message(
                    id, text=f'Индекс массы тела: ожирение 1 степени')
            elif (mass_body_index < 16):
                bot.send_message(
                    id, text=f'Индекс массы тела: выраженный дефицит массы тела')
            elif (34.9 < mass_body_index <= 39.9):
                bot.send_message(
                    id, text=f'Индекс массы тела: ожирение 2 степени')
            elif (39.9 < mass_body_index <= 44.9):
                bot.send_message(
                    id, text=f'Индекс массы тела: ожирение 3 степени')
            elif (44.9 < mass_body_index):
                bot.send_message(
                    id, text=f'Индекс массы тела: ожирение 4 степени')
        result = users_data[id].systolic_blood_pressure + \
            users_data[id].heart_rate + users_data[id].respiratory_rate
    ...
    if state == UserStates.first_scale_end or state == UserStates.second_scale_end:
        ...
        bot.send_message(
            id, text=f'Чтобы начать новый сеанс нажмите на кнопку. ', reply_markup=start_keyboard)
...
def new_bleeding(call):
    ...
    bot.send_message(call.message.chat.id, text='Выберите шкалу',
...
def ask_body_mass(message):
    ...
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        ...
        if not is_float(message.text):
            bot.send_message(
                message.chat.id, text='Данные введены неверно, попробуйте еще раз')
            return
...
@bot.callback_query_handler(func=lambda call: call.data in ('first_scale', 'second_scale'))
def on_scale_selection(call):
    """
    Обрабатывает callback на выбор шкалы
    """

    # Добавляем в словарь данные по выбранной шкале
    match call.data:
        case "first_scale":
            users_scale[call.message.chat.id] = UserScale(FIRST_SCALE_STATES)
            bot.edit_message_text(chat_id=call.message.chat.id,
                                  message_id=call.message.message_id, text='Вы выбрали первую шкалу')
        case "second_scale":
            users_scale[call.message.chat.id] = UserScale(SECOND_SCALE_STATES)
            bot.edit_message_text(chat_id=call.message.chat.id,
                                  message_id=call.message.message_id, text='Вы выбрали вторую шкалу')
...
def cad_query(call):
    ...
    if call.data == 'CAD_NORM':
        ...
        bot.edit_message_text(chat_id=call.message.chat.id,
                              message_id=call.message.message_id, text='Снижение САД на 1-20 мм.рт.ст')
    elif call.data == 'CAD_1':
        users_data[call.message.chat.id].systolic_blood_pressure = 1
        bot.edit_message_text(chat_id=call.message.chat.id,
                              message_id=call.message.message_id, text='Снижение САД на 20-50 мм.рт.ст.')
    elif call.data == 'CAD_2':
        users_data[call.message.chat.id].systolic_blood_pressure = 2
        bot.edit_message_text(chat_id=call.message.chat.id,
                              message_id=call.message.message_id, text='Снижение САД на 50-80 мм.рт.ст.')
    elif call.data == 'CAD_3':
        users_data[call.message.chat.id].systolic_blood_pressure = 3
        bot.edit_message_text(chat_id=call.message.chat.id,
                              message_id=call.message.message_id, text='Снижение САД на более 80 мм.рт.ст.')
...
def hr_query(call):
    ...
    if call.data == 'HR_NORM':
        ...
        bot.edit_message_text(chat_id=call.message.chat.id,
                              message_id=call.message.message_id, text='значения в пределах нормы ')
    elif call.data == 'HR_1':
        users_data[call.message.chat.id].heart_rate = 1
        bot.edit_message_text(chat_id=call.message.chat.id,
                              message_id=call.message.message_id, text='Повышение ЧСС на 1-20 уд/мин.')
    elif call.data == 'HR_2':
        users_data[call.message.chat.id].heart_rate = 2
        bot.edit_message_text(chat_id=call.message.chat.id,
                              message_id=call.message.message_id, text='Повышение ЧСС на 20-40 уд/мин. ')
    elif call.data == 'HR_3':
        users_data[call.message.chat.id].heart_rate = 3
        bot.edit_message_text(chat_id=call.message.chat.id,
                              message_id=call.message.message_id, text='Повышение ЧСС на более 40 уд/мин.')
...
def br_query(call):
    ...
    if call.data == 'BR_1':
        ...
        bot.edit_message_text(chat_id=call.message.chat.id,
                              message_id=call.message.message_id, text='ЧДД менее 26/мин.')
    elif call.data == 'BR_2':
        users_data[call.message.chat.id].respiratory_rate = 1
        bot.edit_message_text(chat_id=call.message.chat.id,
                              message_id=call.message.message_id, text='ЧДД 26-30/мин. ')
    elif call.data == 'BR_3':
        users_data[call.message.chat.id].respiratory_rate = 2
        bot.edit_message_text(chat_id=call.message.chat.id,
                              message_id=call.message.message_id, text='ЧДД более 30/мин. ')
...
def diures_query(call):
    ...
    if call.data == 'DIURES_NORM':
        ...
        bot.edit_message_text(chat_id=call.message.chat.id,
                              message_id=call.message.message_id, text='Диурез более 20-30 мл/ч.')
    elif call.data == 'DIURES_2':
        users_data[call.message.chat.id].diures = 2
        bot.edit_message_text(chat_id=call.message.chat.id,
                              message_id=call.message.message_id, text='Диурез 5-20 мл/ч.')
    elif call.data == 'DIURES_3':
        users_data[call.message.chat.id].diures = 3
        bot.edit_message_text(chat_id=call.message.chat.id,
                              message_id=call.message.message_id, text='Анурия. ')
...
def any_state(message):
    ...
    """
    bot.send_message(
        message.chat.id, "Ошибка. Обратитесь к администратору бота. Начните работу с ботом заново командой /start")
